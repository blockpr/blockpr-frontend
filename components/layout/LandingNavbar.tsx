...
type NavItem  = { label: string; href: string; icon: string; desc?: string; modal?: boolean; scrollId?: string; scrollOffset?: number }
...
const PRODUCTO_ITEMS: NavItem[] = [
  {
    ...
    scrollId: 'soluciones', scrollOffset: 1.2,
  },
  {
  ...
  },
  {
    ...
    desc: 'Plug & play',
    scrollId: 'como-funciona', scrollOffset: 0.5,
  },
  {
    label: 'Seguridad',
    href: '/#seguridad',
    ...
    scrollId: 'seguridad', scrollOffset: 0,
  },
]

const PRODUCTO_CASES: CaseItem[] = [
  { label: 'Educación', desc: 'Diplomas verificables',  href: '/blog/certificados-educativos-verificables',        icon: 'M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5' },
  { label: 'RRHH',      desc: 'Credenciales laborales', href: '/blog/rrhh-verificacion-antecedentes-laborales',     icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
  { label: 'Legal',     desc: 'Documentos probatorios', href: '/blog/legal-contratos-con-prueba-criptografica',     icon: 'M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0012 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c1.01.143 2.01.317 3 .52m-3-.52l2.62 10.726c.122.499-.106 1.028-.589 1.202a5.988 5.988 0 01-2.031.352 5.988 5.988 0 01-2.031-.352c-.483-.174-.711-.703-.59-1.202L18.75 4.971zm-16.5.52c.99-.203 1.99-.377 3-.52m0 0l2.62 10.726c.122.499-.106 1.028-.589 1.202a5.989 5.989 0 01-2.031.352 5.989 5.989 0 01-2.031-.352c-.483-.174-.711-.703-.59-1.202L5.25 4.971z' },
  { label: 'Fintech',   desc: 'Contratos sellados',     href: '/blog/fintech-kyc-documentacion-resistente-fraude', icon: 'M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z' },
]

const RECURSOS_ITEMS: NavItem[] = [
  { label: 'Documentación', href: '/docs', icon: 'M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25', desc: 'Guías técnicas' },
  ...
  { label: 'Contáctanos',   href: '#',              icon: 'M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75',                                    desc: 'Habla con el equipo', modal: true },
]
...
function DropContent({ dropKey, onClose }: { dropKey: DropKey; onClose: () => void }) {
  ...
  const scrollTo = (scrollId: string, scrollOffset: number) => {
    onClose()
    const el = document.getElementById(scrollId)
    if (el) {
      const top = el.getBoundingClientRect().top + window.scrollY + window.innerHeight * scrollOffset
  ...
  return (
    <div style={{ animation: 'navContentIn 0.14s ease forwards' }}>
      ...
      <div className="px-1.5 pb-1.5 grid grid-cols-2">
        {cfg.items.map(({ label, href, icon, desc, modal, scrollId, scrollOffset = 0 }) => {
          const inner = (
            <>
          ...
          if (modal) return (
            <button
              ...
              className="flex items-start gap-3 px-4 py-3.5 rounded-[7px] hover:bg-white/6 transition-colors group text-left bg-transparent border-0 cursor-pointer w-full"
            >
              {inner}
            </button>
          )
          if (scrollId) return (
            <button
              key={label}
              onClick={() => scrollTo(scrollId, scrollOffset)}
              className="flex items-start gap-3 px-4 py-3.5 rounded-[7px] hover:bg-white/6 transition-colors group text-left bg-transparent border-0 cursor-pointer w-full"
            >
              {inner}
            </button>
...
export function LandingNavbar() {
  ...
  return (
    <>
      ...
      <header className="fixed top-0 inset-x-0 z-50 h-16 flex items-center px-4 md:px-8 bg-black border-b border-white/10">
        {/* Logo */}
        ...
        {!isMobile && (
          <div ref={navRef} className="ml-8 flex items-center gap-1 relative">
            ...
            <Link href="/about" className="px-3 py-1.5 rounded-[6px] text-sm text-white hover:text-white/80 hover:bg-white/8 transition-colors">
              Empresa
            </Link>
        ...
        {isMobile && (
          <button
            ...
            className="ml-auto flex flex-col justify-center items-center w-10 h-10 gap-[5px] bg-transparent border-0 cursor-pointer p-0"
      ...
      {isMobile && menuOpen && (
        <div style={{
          ...
          <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '32px 24px', gap: 0 }}>
            {/* Main nav links */}
            {(Object.keys(CONFIGS) as DropKey[]).map(key => (
              <div key={key} style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', paddingBottom: 20, marginBottom: 20 }}>
                <div style={{ fontSize: 11, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 14, fontWeight: 500 }}>
                  {DROP_LABELS[key]}
                </div>
            ...
            <Link href="#precio" onClick={() => setMenuOpen(false)} style={{ fontSize: 18, color: '#fff', textDecoration: 'none', padding: '8px 0 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', marginBottom: 20, fontWeight: 300, letterSpacing: '-0.02em' }}>
              Precio
            </Link>
            <Link href="/about" onClick={() => setMenuOpen(false)} style={{ fontSize: 18, color: '#fff', textDecoration: 'none', padding: '8px 0 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', marginBottom: 28, fontWeight: 300, letterSpacing: '-0.02em' }}>
              Empresa
            </Link>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 'auto' }}>
              <Link
                href="/signup"
                onClick={() => setMenuOpen(false)}
                style={{ display: 'block', padding: '14px 24px', background: '#fff', color: '#000', borderRadius: 6, fontSize: 14, fontWeight: 600, textDecoration: 'none', textAlign: 'center', letterSpacing: '0.01em' }}
              >
                Empezar ahora
              </Link>
              <Link
                href="/login"
                onClick={() => setMenuOpen(false)}
                style={{ display: 'block', padding: '13px 24px', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 6, fontSize: 14, color: '#fff', textDecoration: 'none', textAlign: 'center', letterSpacing: '0.01em', fontWeight: 300 }}
              >
                Iniciar sesión
              </Link>
            </div>
